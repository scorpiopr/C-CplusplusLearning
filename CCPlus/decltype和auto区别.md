...
|---|---|---|---|
| 是否必须初始化 | 是（必须提供初始值） | 否（只需提供表达式） | 是（作为变量声明时必须初始化） |
| 对 const 和 & | 默认丢弃（除非显式指定） | 完美保留 | 完美保留 |
| 核心目的 | 简化冗长的变量声明类型 | 获取表达式或变量的真实类型 | 完美保留原类型的全自动推导 |
| 不适用/无法推导的情况 | ❌ 无初始值时 ❌ 顶层 const/引用需保留时 ❌ 类非静态成员变量初始化时 ❌ 数组 | ❌ 重载函数名 ❌ 未绑定对象的非静态成员函数 ❌ 位域（Bit-field）的引用表达式 | ❌ 非单一变量/非函数返回的复杂表达式 ❌ 前置声明的不完整类型 ❌ 误加外层括号导致变成引用陷阱 |
